...
from .forms import TirForm, JobForm, editJobForm, AngajatForm, editAngajatForm, PoartaForm, editPoartaForm, ComandaForm, ProdusForm, editProdusForm, FirmaForm, editFirmaForm, ALTLForm, Program_tirForm, TransportForm, Produse_comandaForm
...
cx_Oracle.init_oracle_client(lib_dir=r"C:/app/timia/product/21c/dbhomeXE/bin")


def connection():
    conn = cx_Oracle.connect(user=os.environ.get(
        'USER'), password=os.environ.get('PASSWORD'), dsn=os.environ.get('DSN'))
    return conn
...
def TirList(request):
    ...
    search_columns = ["id_tir", "nr_inmatriculare",
                      "nume_sofer", "prenume_sofer", "telefon_sofer"]
...
def addTir(request):
    ...
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Tir VALUES (:id_tir, :nr_inmatriculare, :nume_sofer, :prenume_sofer, :telefon_sofer)", [
                       id_tir, nr_inmatriculare, nume_sofer, prenume_sofer, telefon_sofer])
        conn.commit()
        conn.close()
...
def updateTir(request, id_tir):
    ...
    if request.method == 'GET':
        ...
        row = cursor.fetchone()
        Tir.append({"id_tir": row[0], "nr_inmatriculare": row[1],
                   "nume_sofer": row[2], "prenume_sofer": row[3], "telefon_sofer": row[4]})
        conn.close()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            cursor.execute("UPDATE Tir SET id_tir=:id_tir,  nr_inmatriculare = :nr_inmatriculare, nume_sofer =:nume_sofer, prenume_sofer = :prenume_sofer, telefon_sofer = :telefon_sofer WHERE id_tir = :id", [
                           id_tir, nr_inmatriculare, nume_sofer, prenume_sofer, telefon_sofer, id])
            conn.commit()
        conn.close()
        return redirect('TirList')
...
def addJob(request):
    ...
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Job VALUES (:id_job, :denumire, :salariu)", [
                       id_job, denumire, salariu])
        conn.commit()
        conn.close()
...
def updateJob(request, id_job):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            cursor.execute("UPDATE Job SET  denumire = :denumire, salariu = :salariu WHERE id_job = :id_job", [
                           denumire, salariu, id])
            conn.commit()
        conn.close()
        return redirect('JobList')
    ...
    if request.method == 'GET':
        ...
        for row in cursor.fetchall():
            angajati.append({"id_angajat": row[0], "nume": row[1], "prenume": row[2],
                            "job": row[3], "nr_telefon": row[4], "email": row[5]})
...
def ListAngajat(request):
    ...
    search_columns = ["id_angajat", "nume",
                      "prenume", "job", "nr_telefon", "email"]
...
def addAngajat(request):
    if request.method == 'GET':
        ...
        for row in cursor.fetchall():
            jobs.append(
                {"id_job": row[0], "denumire": row[1], "salariu": row[2]})
        conn.close()
    ...
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Angajat VALUES (:id_angajat, :nume, :prenume, :job, :nr_telefon, :email)", [
                       id_angajat, nume, prenume, job, nr_telefon, email])
        conn.commit()
        conn.close()
...
def updateAngajat(request, id_angajat):
    ...
    if request.method == 'GET':
        cursor.execute(
            "SELECT * FROM Angajat WHERE id_angajat = :id_angajat", [id_angajat])
        row = cursor.fetchone()
        Angajat.append({"id_angajat": row[0], "nume": row[1], "prenume": row[2],
                       "job": row[3], "nr_telefon": row[4], "email": row[5]})
        conn.close()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            cursor.execute("UPDATE Angajat SET  nume = :nume, prenume = :prenume, job = :job, nr_telefon = :nr_telefon, email = :email WHERE id_angajat = :id_angajat", [
                           nume, prenume, job, nr_telefon, email, id])
            conn.commit()
        conn.close()
        return redirect('ListAngajat')
...
def deleteAngajat(request, id_angajat):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "DELETE FROM Angajat WHERE id_angajat = :id_angajat", [id_angajat])
    conn.commit()
    conn.close()
    return redirect('ListAngajat')
...
def ListTura(request):
    ...
    for row in cursor.fetchall():
        ture.append(
            {"id_tura": row[0], "ora_incepere": row[1], "ora_sfarsit": row[2]})
    conn.close()
...
def addPoarta(request):
    ...
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Poarta VALUES (:id_poarta, :stare_poarta)", [
                       id_poarta, stare_poarta])
        conn.commit()
        conn.close()
...
def updatePoarta(request, id_poarta):
    ...
    if request.method == 'GET':
        ...
        cursor.execute(
            "SELECT * FROM Poarta WHERE id_poarta = :id_poarta", [id_poarta])
        row = cursor.fetchone()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            cursor.execute("UPDATE Poarta SET  stare_poarta = :stare_poarta WHERE id_poarta = :id_poarta", [
                           stare_poarta, id])
            conn.commit()
        conn.close()
        return redirect('ListPoarta')
...
def deletePoarta(request, id_poarta):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "DELETE FROM Poarta WHERE id_poarta = :id_poarta", [id_poarta])
    conn.commit()
    conn.close()
    return redirect('ListPoarta')
...
def ListComanda(request):
    ...
    for row in cursor.fetchall():
        comenzi.append(
            {"id_comanda": row[0], "data_comanda": row[1], "tip_comanda": row[2]})
...
def addComanda(request):
    ...
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Comanda VALUES (:id_comanda, :data_comanda, :tip_comanda)", [
...
def deleteComanda(request, id_comanda):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "DELETE FROM Comanda WHERE id_comanda = :id_comanda", [id_comanda])
    conn.commit()
    conn.close()
    return redirect('ListComanda')
...
def ListProdus(request):
    ...
    produse = []
    for row in cursor.fetchall():
        produse.append({"id_produs": row[0], "id_firma": row[1], "nume_produs": row[2],
                       "nr_paleti": row[3], "produse_per_palet": row[4], "tip_produs": row[5], "pret_produs": row[6]})
    ...
    search_columns = ["id_produs", "id_firma", "nume_produs",
                      "nr_paleti", "produse_per_palet", "tip_produs", "pret_produs"]
...
def addProdus(request):
    ...
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Produs_stoc VALUES (:id_produs, :id_firma, :nume_produs, :nr_paleti, :produse_per_palet, :tip_produs, :pret_produs)", [
                       id_produs, id_firma, nume_produs, nr_paleti, produse_per_palet, tip_produs, pret_produs])
        conn.commit()
        conn.close()
...
def updateProdus(request, id_produs):
    ...
    if request.method == 'GET':
        ...
        cursor.execute(
            "SELECT * FROM Produs_stoc WHERE id_produs = :id_produs", [id_produs])
        produs = []
        for row in cursor.fetchall():
            produs.append({"id_produs": row[0], "id_firma": row[1], "nume_produs": row[2], "nr_paleti": row[3],
                          "produse_per_palet": row[4], "tip_produs": row[5], "pret_produs": row[6]})
        opt = ['Detergenti', 'Bauturi', 'Sarate', 'Parfumerie', 'Dulce']
        conn.close()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            cursor.execute("UPDATE Produs_stoc SET  id_firma = :id_firma, nume_produs = :nume_produs, nr_paleti = :nr_paleti, produse_per_palet = :produse_per_palet, tip_produs = :tip_produs, pret_produs = :pret_produs WHERE id_produs = :id_produs", [
                           id_firma, nume_produs, nr_paleti, produse_per_palet, tip_produs, pret_produs, id_produs])
            conn.commit()
        conn.close()
        return redirect('ListProdus')
...
def deleteProdus(request, id_produs):
    if request.method == 'GET':
        ...
        cursor = conn.cursor()
        cursor.execute(
            "DELETE FROM Produs_stoc WHERE id_produs = :id_produs", [id_produs])
        conn.commit()
        conn.close()
        return redirect('ListProdus')
...
def ListFirma(request):
    ...
    firme = []
    for row in cursor.fetchall():
        firme.append({"id_firma": row[0], "nume": row[1], "data_semnare_contract": row[2],
                     "data_incheiere_contract": row[3], "email": row[4], "contact_telefon": row[5]})
    ...
    search_columns = ["id_firma", "nume", "data_semnare_contract",
                      "data_incheiere_contract", "email", "contact_telefon"]
...
def addFirma(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            id_firma = form.cleaned_data.get("id_firma")
            nume = form.cleaned_data.get("nume")
            data_semnare_contract = form.cleaned_data.get(
                "data_semnare_contract")
            data_incheiere_contract = form.cleaned_data.get(
                "data_incheiere_contract")
            email = form.cleaned_data.get("email")
            contact_telefon = form.cleaned_data.get("contact_telefon")
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Firma VALUES (:id_firma, :nume, :data_semnare_contract, :data_incheiere_contract, :email, :contact_telefon)", [
                       id_firma, nume, data_semnare_contract, data_incheiere_contract, email, contact_telefon])
        conn.commit()
        conn.close()
...
def updateFirma(request, id_firma):
    ...
    if request.method == 'GET':
        cursor.execute(
            "SELECT * FROM Firma WHERE id_firma = :id_firma", [id_firma])
        firma = []
        for row in cursor.fetchall():
            firma.append({"id_firma": row[0], "nume": row[1], "data_semnare_contract": row[2],
                         "data_incheiere_contract": row[3], "email": row[4], "contact_telefon": row[5]})
        conn.close()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            nume = str(form.cleaned_data.get("nume"))
            data_semnare_contract = str(
                form.cleaned_data.get("data_semnare_contract"))
            data_incheiere_contract = str(
                form.cleaned_data.get("data_incheiere_contract"))
            email = str(form.cleaned_data.get("email"))
            contact_telefon = str(form.cleaned_data.get("contact_telefon"))
            cursor.execute("UPDATE Firma SET  nume = :nume, data_semnare_contract = :data_semnare_contract, data_incheiere_contract = :data_incheiere_contract, email = :email, contact_telefon = :contact_telefon WHERE id_firma = :id_firma", [
                           nume, data_semnare_contract, data_incheiere_contract, email, contact_telefon, id_firma])
            conn.commit()
        conn.close()
        return redirect('ListFirma')
...
def deleteFirma(request, id_firma):
    if request.method == 'GET':
        ...
        cursor = conn.cursor()
        cursor.execute(
            "DELETE FROM Firma WHERE id_firma = :id_firma", [id_firma])
        conn.commit()
        conn.close()
        return redirect('ListFirma')
...
def ListALTL(request):
    ...
    for row in cursor.fetchall():
        altl.append(
            {"id_poarta": row[0], "id_angajat": row[1], "id_tura": row[2], "data": str(row[3])})
    for alt in altl:
...
def addALTL(request):
    if request.method == 'GET':
        ...
        for row in cursor.fetchall():
            angajati.append(
                {"id_angajat": row[0], "nume": row[1], "prenume": row[2]})
    ...
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Angajat_lucreaza_tura_la_poarta VALUES (:id_poarta, :id_angajat, :id_tura, :data)", [
                       id_poarta, id_angajat, id_tura, data])
        conn.commit()
        conn.close()
...
def updateALTL(request, id_poarta, id_angajat, id_tura, data):
    ...
    if request.method == 'GET':
        cursor.execute("SELECT * FROM Angajat_lucreaza_tura_la_poarta WHERE id_poarta = :id_poarta AND id_angajat = :id_angajat AND id_tura = :id_tura AND data = :data",
                       [id_poarta, id_angajat, id_tura, data])
        altl = []
        for row in cursor.fetchall():
            altl.append(
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            id_tura = form.cleaned_data.get("id_tura")
            data = form.cleaned_data.get("data")
            cursor.execute("UPDATE Angajat_lucreaza_tura_la_poarta SET  id_poarta = :idp, id_angajat = :ida, id_tura = :id_tura, data = :data WHERE id_poarta = :id_poarta AND id_angajat = :id_angajat AND id_tura = :idt AND data = :d", [
                           id_poarta, id_angajat, id_tura, data, idp, ida, idt, d])
            conn.commit()
        conn.close()
        return redirect('ListALTL')
...
def deleteALTL(request, id_poarta, id_angajat, id_tura, data):
    conn = connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM Angajat_lucreaza_tura_la_poarta WHERE id_poarta = :id_poarta AND id_angajat = :id_angajat AND id_tura = :id_tura AND data =:data", [
                   id_poarta, id_angajat, id_tura, data])
    conn.commit()
    conn.close()
    return redirect('ListALTL')
...
def ListProgram_tir(request):
    ...
    program_tir = []
    for row in cursor.fetchall():
        program_tir.append({"id_tir": row[0], "id_poarta": row[1], "intrare": str(
            row[2]), "iesire": str(row[3])})
    for alt in program_tir:
        date_obj = datetime.datetime.strptime(
            alt['intrare'], '%Y-%m-%d  %H:%M:%S')
        alt['intrare'] = date_obj.strftime('%d-%b-%Y %H:%M:%S')
        if alt['iesire'] != 'None':
            date_obj = datetime.datetime.strptime(
                alt['iesire'], '%Y-%m-%d  %H:%M:%S')
...
def addProgram_tir(request):
    ...
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Program_tir VALUES (:id_tir, :id_poarta, :intrare, :iesire)", [
                       id_tir, id_poarta, intrare, iesire])
        conn.commit()
        conn.close()
...
def updateProgram_tir(request, id_tir, id_poarta, intrare):
    ...
    if request.method == 'GET':
        cursor.execute(
            "SELECT * FROM Program_tir WHERE id_tir = :id_tir AND id_poarta = :id_poarta", [id_tir, id_poarta])
        program_tir = []
        for row in cursor.fetchall():
            program_tir.append({"id_tir": row[0], "id_poarta": row[1], "intrare": str(
                row[2]), "iesire": str(row[3])})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            cursor.execute("UPDATE Program_tir SET  id_tir = :idt, id_poarta = :idp, intrare = :intrare, iesire = :iesire WHERE id_tir = :id_tir AND id_poarta = :id_poarta AND intrare=:intr", [
                           id_tir, id_poarta, intrare, iesire, idt, idp, intr])
            conn.commit()
        conn.close()
        return redirect('ListProgram_tir')
...
def deleteProgram_tir(request, id_tir, id_poarta, intrare):
    ...
    cursor.execute("DELETE FROM Program_tir WHERE id_tir = :id_tir AND id_poarta = :id_poarta AND intrare=:intrare", [
                   id_tir, id_poarta, intrare])
    conn.commit()
    conn.close()
    return redirect('ListProgram_tir')
...
def ListTransport(request):
    ...
    for row in cursor.fetchall():
        transporturi.append(
            {"id_tir": row[0], "id_comanda": row[1], "distanta": row[2]})
    cursor.execute("SELECT id_tir from Tir")
    tiruri = []
    for row in cursor.fetchall():
...
def addTransport(request):
    ...
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Transport VALUES (:id_tir, :id_comanda, :distanta)", {
                       'id_tir': id_tir, 'id_comanda': id_comanda, 'distanta': distanta})
        conn.commit()
        conn.close()
...
def updateTransport(request, id_tir, id_comanda):
    ...
    if request.method == 'GET':
        cursor.execute(
            "SELECT * FROM Transport WHERE id_tir = :id_tir AND id_comanda = :id_comanda", [id_tir, id_comanda])
        Transporturi = []
        for row in cursor.fetchall():
            Transporturi.append(
                {"id_tir": row[0], "id_comanda": row[1], "distanta": str(row[2])})
        conn.close()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            id_comanda = form.cleaned_data.get("id_comanda")
            distanta = form.cleaned_data.get("distanta")
            cursor.execute("UPDATE Transport SET id_tir = :id_tir, id_comanda=:id_comanda, distanta = :distanta WHERE id_tir = :idt AND id_comanda =:idc", [
                           id_tir, id_comanda, distanta, idt, idc])
            conn.commit()
        conn.close()
        return redirect('ListTransport')
...
def deleteTransport(request, id_tir, id_comanda):
    conn = connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM Transport WHERE id_tir = :id_tir AND id_comanda=:id_comanda", [
                   id_tir, id_comanda])
    conn.commit()
    conn.close()
    return redirect('ListTransport')
...
def ListProduse_comanda(request):
    ...
    prods_comanda = []
    for row in cursor.fetchall():
        prods_comanda.append(
            {"id_produs": row[0], "id_comanda": row[1], "nr_paleti": row[2]})
...
def addProduse_comanda(request):
    ...
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Produse_comanda VALUES (:id_produs, :id_comanda, :nr_paleti)", {
                       'id_produs': id_produs, 'id_comanda': id_comanda, 'nr_paleti': nr_paleti})
        conn.commit()
        conn.close()
...
def deleteProduse_comanda(request, id_produs, id_comanda):
    conn = connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM Produse_comanda WHERE id_produs = :id_produs AND id_comanda=:id_comanda", [
...
""")  # nu se justifica left join, cand am conditia id_firma=2, nu se pot compara oricum valorile null, deci nu are rost
    cereri = []
    for row in cursor.fetchall():
        cereri.append({"id_comanda": row[0], "data_comanda": row[1], "tip_comanda": row[2], "id_firma": row[3],
                      "nume_firma": row[4], "id_produs": row[5], "nume_produs": row[6], "nr_paleti": row[7]})
    conn.close()
    for alt in cereri:
...
AND SUM(nr_paleti)>100""")
    cereri = []
    for row in cursor.fetchall():
        cereri.append({"tip_produs": row[0], "id_firma": row[1], "nume_firma": row[2],
                      "nr_total_paleti": row[3], "pret_mediu": row[4], "nr_produse": row[5]})
    conn.close()
...
def ListView1(request):
    ...
    vizualizari = []
    for row in cursor.fetchall():
        vizualizari.append({"id_angajat": row[0], "nume": row[1], "prenume": row[2], "job": row[3],
                           "nr_telefon": row[4], "email": row[5], "denumire": row[6], "salariu": row[7]})
...
def addView1(request):
    ...
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Vizualizare_compusa(id_angajat, nume, prenume, job, nr_telefon, email) VALUES (:id_angajat, :nume, :prenume, :job, :nr_telefon, :email)", [
                       id_angajat, nume, prenume, job, nr_telefon, email])
        conn.commit()
        conn.close()
...
def updateView1(request, id_angajat):
    ...
    if request.method == 'GET':
        ...
        for row in cursor.fetchall():
            jobs.append(
                {"id_job": row[0], "denumire": row[1], "salariu": row[2]})
        cursor.execute(
            "SELECT * FROM Vizualizare_compusa WHERE id_angajat = :id_angajat", [id_angajat])
        row = cursor.fetchone()
        angajat.append({"id_angajat": row[0], "nume": row[1], "prenume": row[2], "job": row[3],
                       "nr_telefon": row[4], "email": row[5], "denumire": row[6], "salariu": row[7]})
    ...
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE Vizualizare_compusa SET nume=:nume, prenume=:prenume, job=:job, nr_telefon=:nr_telefon, email=:email WHERE id_angajat=:id_angajat", [
                       nume, prenume, job, nr_telefon, email, id_angajat])
        conn.commit()
        conn.close()
        return redirect('ListView1')
...
def deleteView1(request, id_angajat):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "DELETE FROM Vizualizare_compusa WHERE id_angajat=:id_angajat", [id_angajat])
    conn.commit()
    conn.close()
    return redirect('ListView1')
...
def ListView2(request):
    ...
    vizualizari = []
    for row in cursor.fetchall():
        vizualizari.append({"id_firma": row[0], "Nume_Firma": row[1],
                           "tip_produs": row[2], "Nr_total_paleti": row[3], "Nr_produse": row[4]})
